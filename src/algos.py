...
def run_rf_nash_ac(env, 
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            # Select and perform an action
            dist_1 = agent_1.select_action(state)
            dist_2 = agent_2.select_action(state, dist_1.detach())
            ...
            # agent_1.buffer.push(state, action_1, action_2, torch.unsqueeze(dist_1, 0).detach(), next_state, reward_1)
            # agent_2.buffer.push(state, action_2, action_1, torch.unsqueeze(dist_2, 0).detach(), next_state, reward_2)
            agent_1.buffer.push(state, action_1,  action_2, torch.unsqueeze(dist_2, 0).detach(), next_state, reward_1)
            agent_2.buffer.push(state, action_2,  action_1, torch.unsqueeze(dist_1, 0).detach(), next_state, reward_2)
            ...
            agent_1.transition = [state, torch.unsqueeze(dist_2, 0)]
            agent_2.transition = [state, torch.unsqueeze(dist_1, 0)]
...
def run_sr_nash_ac(env, 
    ...
    for i_episode in range(num_episodes):
        ...
        episode_rewards = []
        obs, _ = env.reset()
        curr_state = torch.tensor(obs, dtype=torch.float32 , device=device)
        curr_actions = torch.tensor([-1, -1], dtype=torch.float32 , device=device)
        curr_state_and_actions = torch.cat([curr_state.flatten(), curr_actions], dim=0)

        for t in count():
            if use_history:
                history = agent_1.aggregate_history().float()
                history = torch.unsqueeze(history, dim=0)
                state = torch.cat([curr_state_and_actions.reshape(1, 1, curr_state_and_actions.shape[0]), history], dim=1)
            ...
            obs, rewards, done, _, _ = env.step((action_1.item(), action_2.item()))
            curr_state = torch.tensor(obs, dtype=torch.float32 , device=device)
            reward_1 = torch.tensor([rewards[0]], dtype=torch.float32 , device=device)
            reward_2 = torch.tensor([rewards[1]], dtype=torch.float32 , device=device)
            ...
            # Store the transition in memory
            agent_1.buffer.push(last_state, action_1,  action_2, torch.unsqueeze(dist_2, 0).detach(), curr_state, reward_1)
            agent_2.buffer.push(last_state, action_2,  action_1, torch.unsqueeze(dist_1, 0).detach(), curr_state, reward_2)
            agent_1.buffer.push(last_state, action_2,  action_1, torch.unsqueeze(dist_1, 0).detach(), curr_state, reward_2)
            agent_2.buffer.push(last_state, action_1,  action_2, torch.unsqueeze(dist_2, 0).detach(), curr_state, reward_1)

            agent_1.o_actor.load_state_dict(agent_2.actor.state_dict())
            agent_2.o_actor.load_state_dict(agent_1.actor.state_dict())
            agent_1.o_actor_head.load_state_dict(agent_2.actor_head.state_dict())
            agent_2.o_actor_head.load_state_dict(agent_1.actor_head.state_dict())
            ...
            # Perform one step of the optimization (on the reward and policy networks)
            if np.random.binomial(1, 1/2):
                game_value_1, value_loss_1 = agent_1.optimize_model(is_inducer=np.random.binomial(1, 1/2), cum_steps=cum_steps)
                game_value_2, value_loss_2 = agent_2.optimize_model(is_inducer=np.random.binomial(1, 1/2), cum_steps=cum_steps)
            else:
                game_value_2, value_loss_2 = agent_2.optimize_model(is_inducer=np.random.binomial(1, 1/2), cum_steps=cum_steps)
                game_value_1, value_loss_1 = agent_1.optimize_model(is_inducer=np.random.binomial(1, 1/2), cum_steps=cum_steps)
