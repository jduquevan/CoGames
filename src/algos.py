...
def run_rf_nash_ac(env, 
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            # agent_1.buffer.push(state, action_1, action_2, torch.unsqueeze(dist_1, 0).detach(), next_state, reward_1)
            # agent_2.buffer.push(state, action_2, action_1, torch.unsqueeze(dist_2, 0).detach(), next_state, reward_2)
            agent_1.buffer.push(state,  torch.unsqueeze(dist_1, 0).detach(),  torch.unsqueeze(dist_2, 0).detach(), reward_1)
            agent_2.buffer.push(state,  torch.unsqueeze(dist_2, 0).detach(),  torch.unsqueeze(dist_1, 0).detach(), reward_2)
...
def run_sr_nash_ac(env, 
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            agent_1.buffer.push(state, torch.unsqueeze(dist_1, 0).detach(),  torch.unsqueeze(dist_2, 0).detach(), reward_1)
            agent_2.buffer.push(state, torch.unsqueeze(dist_2, 0).detach(),  torch.unsqueeze(dist_1, 0).detach(), reward_2)
            agent_1.buffer.push(state, torch.unsqueeze(dist_2, 0).detach(),  torch.unsqueeze(dist_1, 0).detach(), reward_2)
            agent_2.buffer.push(state, torch.unsqueeze(dist_1, 0).detach(),  torch.unsqueeze(dist_2, 0).detach(), reward_1)
            ...
            # Perform one step of the optimization (on the reward and policy networks)
            if np.random.binomial(1, 1/2):
                game_value_1, value_loss_1 = agent_1.optimize_model(is_inducer=np.random.binomial(1, 1/2))
                game_value_2, value_loss_2 = agent_2.optimize_model(is_inducer=np.random.binomial(1, 1/2))
            else:
                game_value_2, value_loss_2 = agent_2.optimize_model(is_inducer=np.random.binomial(1, 1/2))
                game_value_1, value_loss_1 = agent_1.optimize_model(is_inducer=np.random.binomial(1, 1/2))
